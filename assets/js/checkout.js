...
//APPLY COUPON CODE
// Predefined coupon codes and their corresponding discount percentages
const couponCodes = {
  DISCOUNT10: 10,
  HIHIHIHI: 20,
  HAHHAHAHA: 25,
...
function applyCoupon() {
  ...
  let initialTotalAmount = 230; // Assume the initial total amount is $230
  let totalAmount = initialTotalAmount; // Store the initial total amount

  // Check if the entered coupon code matches any predefined coupon code
  if (couponCodes.hasOwnProperty(couponInput)) {
    const discountPercentage = couponCodes[couponInput];
    const discountAmount = (totalAmount * discountPercentage) / 100;
    const newTotalAmount = totalAmount - discountAmount;
    totalElement.innerHTML = `<p>Rs. ${newTotalAmount.toFixed(2)}$</p>`;

    document.getElementById(
      "success"
    ).innerText = `Coupon applied! You saved ${discountPercentage}%.`;
    document.getElementById("error").innerText = "";
  } else {
    totalElement.innerHTML = `<p>Rs. ${initialTotalAmount.toFixed(2)}$</p>`; // Reset to initial amount
    document.getElementById("error").innerText = "Invalid coupon code.";
